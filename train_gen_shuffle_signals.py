...
def generate_new_signals(dataset, num_signals, num_rep, epoch_length, num):
    filtered_dataset = [(train_signal, train_label) for train_signal,  train_label in dataset if train_label == num]
    filtered_signals = np.array([train_signal for train_signal, _ in filtered_dataset])
...
class CNN(pl.LightningModule):
  def __init__(self):
    ...
    self.n_dim = 23392
    self.fc1 = nn.Sequential(nn.Linear(self.n_dim, 1))
...
for i in range(10):
    df = pd.read_csv('nirs with MRI outcome for CNN_10hours.csv', header = None)
    label = pd.read_csv('MRI outcome for CNN_10hours.csv', header = None)
    ...
    val_signal = train_signal.reshape(-1,1,5890)
    ...
    for i in range(10):
        if i <4:
            ...
            df_combined_signals = generate_new_signals(dataset,2, 2, 589, 1)
